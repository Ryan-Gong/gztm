// miniprogram/pages/repairs/add/add.js
const app = getApp();
...
Page({
  ...
  selectFacility:function(e){
    ...
    //console.log('value:' + e.detail.value + ';txt:' + this.data.facilityValue);
    //如果选择项的值!=0 且 不是无关联设备
    if (e.detail.value != 0 && this.data.facilityValue !='无关联设备'){
      //使用map映射循环pkgRelation
      let sbbh = ['无关联设备'];
      that.data.pkgRelation.map(function (item, index, array) {
        //如果循环列mc属性==设备名称，则 返回该列的设备编号
        if (item.mc == that.data.facility[e.detail.value]){
          sbbh.push(item.bh);
        }
      });
      //console.log(sbbh);
